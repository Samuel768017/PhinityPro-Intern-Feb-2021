...
async def user_login(user_id: int, db: Session = Depends(get_db)):
...
def get_user_login(username: str,password:str,db : Session = Depends(get_db)):
    ...
    if db_user is None:
        raise HTTPException(status_code=404, detail="credintials not found")
    return db_user
...
async def user_login(user: schemas.UserCreate, db: Session = Depends(get_db)):
    db_user = crud.get_user_login(db,usernamec = user.username,password=user.password)
    if db_user:

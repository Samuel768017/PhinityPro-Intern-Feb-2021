...
def create_user(db: Session, user: schemas.UserCreate):
    ...
    db_user = User(title=user.title,firstname=user.firstname, lastname=user.lastname, username=user.username, email=user.email, password=fake_hashed_password,status=user.status)
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    return db_user
